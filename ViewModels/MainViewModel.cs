...
namespace ChatRWKV_PC.ViewModels
{
    public class MainViewModel : BaseViewModel
    {
        ...
        public bool IsSpeed
        {
            ...
            set
            {
                ...
                if (value)
                {
                    ...
                    {
                        System.Windows.MessageBox.Show("没有rwkv依赖包，无法开启加速！");
                        _IsSpeed = false;  
                    }
                }
                else
        ...
        public BtnCommand ConverterModelCommand { get => new BtnCommand(param =>
        {
            ...
            {
                System.Windows.MessageBox.Show("模型文件不存在！！！");
                return;
            }
            //检测转换文件是否存在
        ...
        public BtnCommand SendMessageCommand
        {
            get => new BtnCommand(async param =>
            {
                ...
                if (RunStatus != 1)
                {
                    System.Windows.MessageBox.Show("未启动ChatRWKV");
                    return;
                }
        ...
        public BtnCommand CommandDesc
        {
            get => new BtnCommand(param =>
            {
                System.Windows.MessageBox.Show(@"+ --> 让机器人换个回答
+prompt --> 加载一个新的提示。
...
+++ --> 继续 +gen / +i 的回答
++ --> 换个 +gen / +i 的回答
+qq和+qa在最新新的chat.py中说明已经去掉，但是代码中未去掉，建议使用+i来提问"
, "命令参数说明");
            });
        }
        ...
        public MainViewModel()
        {
            ...
            updateUtil.CheckUpdateVersion("ChatRWKV_PC桌面版");
            GetLibVersion();
